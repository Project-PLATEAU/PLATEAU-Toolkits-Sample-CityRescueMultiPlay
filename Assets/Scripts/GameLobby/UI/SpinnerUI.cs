...
namespace LobbyRelaySample.UI
{
    ...
    public class SpinnerUI : UIPanelBase
    {
        ...
        bool AllLobbiesAreLocked()
        {
            foreach (System.Collections.Generic.KeyValuePair<string, LocalLobby> lobby in Manager.LobbyList.CurrentLobbies)
            {
                if (!lobby.Value.Locked.Value)
                {
                    return false;
                }
            }
        ...
        void QueryStateChanged(LobbyQueryState state)
        {
            try
            {
                ...
                {
                    // the second check is for locked lobbies
                    if (Manager.LobbyList.CurrentLobbies.Count < 1 || (Manager.LobbyList.CurrentLobbies.Count >= 1 && AllLobbiesAreLocked()))
                    {
                        m_noServerText.Show();
                    }
                    else
