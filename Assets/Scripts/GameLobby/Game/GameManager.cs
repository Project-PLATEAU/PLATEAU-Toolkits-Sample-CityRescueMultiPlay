...
namespace LobbyRelaySample
{
    ...
    public class GameManager : MonoBehaviour
    {
        ...
        public void SetLocalUserName(string name)
        {
            if (string.IsNullOrWhiteSpace(name))
            {
                LogHandlerSettings.Instance.SpawnErrorPopup(
                    "‹ó”’‚Ì–¼‘O‚ðŽg—p‚·‚é‚±‚Æ‚Í‚Å‚«‚Ü‚¹‚ñ"); // Lobby error type, then HTTP error type.
                return;
            }
        ...
        public void SetLocalUserStatus(PlayerStatus status)
        {
            ...
            {
                if (player.ID.Value == m_LocalUser.ID.Value)
                {
        ...
        public void UIChangeMenuState(GameState state)
        {
            ...
            if (isQuittingGame)
            {
                //If we were in-game, we want to directly move to the Menu screen to allow for reconnection
